...
- **[docs/GIT_COMMIT_WORKFLOW.md](docs/GIT_COMMIT_WORKFLOW.md)** — review, test, commit, push.
- **[docs/COMMIT_AND_DATABASE_MIGRATIONS.md](docs/COMMIT_AND_DATABASE_MIGRATIONS.md)** — Supabase `db push` when migrations change.
...
- **Persistence** — After each move the full `GameState` (including `history`) is written to **`localStorage`** key `wild-spider-game-v1` for refresh recovery. Logged-in bootstrap **prefers that local snapshot** when present; the server save is used only when local is empty (see spec).
