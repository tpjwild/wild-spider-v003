...
- **numberOfColumns:** The number of **deal columns** in the initial tableau layout (01–12 in formatted seeds). This value is stored as `config.columns` and does **not** cap how many physical tableau columns may exist later (e.g. after **Extra Column** inserts). The live tableau length is `columns.length`.
...
- **numberOfSuits** (tableau only): **4** = normal suit matching on tableau runs; **2** = every regular card is treated as **halfWild** (red H/D, black S/C) for tableau run validation; **1** = every regular card is treated as **wild** for tableau runs. Does **not** change effect **badges** (powers may become largely redundant in 1- or 2-suit modes). **Foundation** always uses physical suit and rank only (wild / halfWild and **numberOfSuits** are ignored there). Stored on `config.numberOfSuits`; encoded in formatted seeds as **S** (see Setup).

### Setup

Uses two standard 52‑card decks with each of the 2 decks containing from zero to 4 jokers (104 to 112 cards total).

**Canonical game seed (formatted):** `CC-DDD-S-XXX-SSSSSSSSSSSSSS` with **hyphens required** when displaying or entering:

- **CC** — two digits, number of tableau columns (01–12).
- **DDD** — three digits, number of deals (at least 005; **CC × DDD** must be ≤ 104).
- **S** — one digit: **numberOfSuits** for tableau rules — **1**, **2**, or **4** only.
...
- **Legacy formatted seeds** without **S** (`CC-DDD-XXX-SSSS…`) are still accepted and imply **4** suits.
...
**Alignment detection (implementation):** After each successful tableau move, foundation move, deal, or **Card Swap** power commit, the engine scans for newly aligned sets (`findAlignedSets` / `findNewlyAlignedSets` in `src/engine/setAlignment.ts`). A set key is `${deckNum}-${suit}` (e.g. `1-H` = deck 1 hearts). **Tableau:** column index **0** is the **bottom** card of the pile; a valid triple has **King** at `i`, **Queen** at `i+1`, **Jack** at `i+2` (Jack immediately above Queen above King), including triples formed in the middle of a column. Face-up and face-down courts both count. **Foundation:** the top three cards of a pile must be **King** (top), **Queen**, **Jack** (bottom of the triple), same suit and deck. The first time a set key aligns, `createShelfSetPowerEntry` appends a `{ kind: "set", … }` shelf entry and records the key in **`alignedSetKeys`**. Undo of the creating action removes those entries via optional **`setPowersAdded`** on the relevant history entry (`src/engine/history.ts`, `src/engine/setPowers.ts`).
...
When a targeted power is triggered, if the user clicks on an invalid target, clicks anywhere on the **Game Bar**, or hits **Escape** then the power’s triggering is canceled and no charge is consumed. Pressing **Shift** while in **Power Target** mode also cancels targeting (no charge consumed).
...
- A card in the deck popup: When the armed power’s `targetKinds` include a deck-popup kind, **pointer enter** on the **Deck** game-bar control opens the **Deck Popup**; click the card in the popup. Moving the pointer **off** the open popup (backdrop and panel) **closes** the popup but **does not** cancel targeting. **Click** on **Deck** / **Stock** on the game bar while targeting cancels only (see **Game Bar**); **clicking the popup backdrop** while targeting still cancels targeting (no charge).
- A card in the stock popup: When the armed power’s `targetKinds` include `stockPopupCard`, **pointer enter** on **Stock** opens the **Stock Popup**; same hover-dismiss and game-bar rules as the deck popup.
- A non-empty foundation pile: Selected by clicking the foundation pile (**Foundation return** only; pile must have a legal tableau placement or the commit is rejected without spending a charge).
- A joker or set power instance on the shelf: Selected by clicking on the item on the shelf.

**Card swap** (two-step): After arming, click one valid card (tableau, Stock popup, or Deck popup — not foundation or shelf), then a second valid card to swap positions and **face-up / face-down state** at those slots (stock counts as face-down). Clicking the same card twice cancels targeting without spending a charge.
...
- **Duration:** Each joker in the deck-pair catalog may set **`initialDuration`**: **`null`** (default) means applied effects are **permanent** until removed; a **positive number** means the effect expires after that many **player moves** (tableau move, foundation move, or deal). **Arming** a targeted power (double-click) is **not** a move and does **not** tick durations. **Committing** any power (immediate or targeted, including move-only powers) **is** a move: `tickEffectDurationsOnTargetCommit` in `src/engine/effects.ts` runs once with exclusions for card/column effects and extra-column parent links **added on that commit** (they start at full duration). All **other** timed card effects, column effects, and **Extra Column** links tick once on commit. `power_trigger` history does **not** use the generic `appendHistory` move tick path (handlers apply the commit tick explicitly). Timed effects also tick down after each tableau move, foundation move, and deal. **Undo** of such a move restores **`timedEffectsBeforeTick`** on that history entry (card effects, column effects, and extra-column link timers as they were immediately before that move’s tick).
- **Column scope:** While a card remains in a column, **column effects** apply to **every** card in that column (in addition to any effects on the card itself), when the effect’s definition has **`columnAffectsTableauCards: true`** in `src/content/effectDefinitions.ts` (`columnEffectAffectsTableauCards`). **`extraColumn`** is **`false`** — topology and column-holder UI only, not inherited onto cards.
...
- **Duration timer:** When at least one timed effect has **moves remaining &gt; 0**, a **numeric timer chip** appears as the **leftmost** badge in that row (still top-right as a group). It is **not** folded into the icon-count collapse. **Hidden** when the soonest remaining ticks are **0** or there are no timed effects. **Cards** (tableau, deck popup, stock popup): timer uses **card** badge colours and reflects the soonest **card-scoped** timed effect only (`soonestCardEffectTicks` in `src/lib/effectDurationTicks.ts`). Badge order on cards: **timer → card effect icons → column effect icons** (column icons use the lighter column chip). **Column badge holders:** timer uses **column** badge colours and reflects the soonest among timed **columnEffects** on that column and, when this column **parents** an Extra Column link, that link’s `movesRemaining` (`soonestColumnHolderTicks`). **Extra-child** holders keep the lighter green strip but **do not** show the parent link timer (the parent column holder shows it).
- **Tableau column:** Each effect on a column adds a badge in that column’s **badge holder** (see Tableau under Game View), using the **column** ring style on the lighter chip. Icons live in `public/gameArt/shared/effect-badges/{effectId}.svg` (see `effectBadgeIconPath` in `src/constants/gameArtPaths.ts`). Up to **`maxEffectBadgesShownIndividually`** (default **3** in `dimensions.ts`) effects **per scope** (card vs column on cards; column-only on holders) show as separate icons; above that per scope, one numeric count badge for that scope. Each icon or count badge has a tooltip (effect labels from `effectDefinitions.ts`; column-scoped entries append “(column)”).
...
**skip1:** For tableau runs and placement, the card’s rank may be treated as its **printed** rank, **one higher**, or **one lower** (exactly **one** of those per card per run slice or placement check). Neighbors in a run must still differ by exactly **one** in **effective** rank.

**skip2:** Same as **skip1**, but the card may also be treated as **±2** ranks from printed (printed, ±1, and ±2 are all allowed choices; still one choice per card per run slice).
...
**Column display labels** (UI): Deal column **i** → `Column ${String(i+1).padStart(2,"0")}` (e.g. `Column 01`). Extra children chained from that deal root use `.01`, `.02`, … in left-to-right chain order: `Column 01`, `Column 01.01`, `Column 01.02`. **Foundation name plate** (inspect): timed effects show `Label (N)`; any column that **parents** an active link (deal column or chain-parent extra-child) shows heading + `Extra Column (N)` for **that** link under Column Effects; **leaf** extra-child hover shows heading and joker column effects only (no `Extra Column` line — the parent column’s badge holder shows the link timer). **Badges:** Parent with active link shows the Extra Column icon (`public/gameArt/shared/effect-badges/extraColumn.svg`) in that column’s badge holder (`columnHolderEffectBadgeEntries`) plus the **duration timer** when the link has ticks remaining. Extra-child column holders use a lighter green strip (`tableauExtraChildBadgeHolderBackground` / `tableauExtraChildBadgeHolderBorder` in `colors.ts`) without the former top-left link timer.
...
| `jokerFoundationReturn` | Targeted | *(move only)* | non-empty foundation pile with a legal tableau placement |
| `jokerCardSwap` | Targeted | *(move only)* | two cards: tableau, Stock popup, or Deck popup (not foundation/shelf); second click swaps |
...
| **3–4 (black)** | **Targeted** | Apply **transparent** to **one** valid target: any **face-down** card on the **tableau**; **any** card in the **Stock popup**; any card in the **Deck popup** that is **not** shown face-up (i.e. still in stock or face-down on tableau in that popup). After arming a targeted power, **hovering** the **Deck** or **Stock** button opens only the popup(s) allowed by that power’s `targetKinds` (`powerTargetsDeckPopup` / `powerTargetsStockPopup` in `powerDefinitions.ts`); e.g. **column** and **foundation** powers do not open either popup on hover. Invalid target or **Escape** cancels without spending a charge. |
...
### Stage 5 set powers (initial registry)

**Set power** ids, targeting, and **`initialCharges` / `initialDuration`** for each aligned court set (deck 1 or 2 × suit S/C/D/H) are authored **with** court names/bios in each deck-pair module via `themedSets()` / `baseSets()` in `src/content/deckPairs/builders.ts` (one object per suit: J/Q/K copy plus `powerId`, `initialCharges`, `initialDuration`). Runtime `setPowers` on `DeckInPair` is built from that input; lookup uses `setPowerDefinitionForSet` / `setPowerForSet` in `src/content/deckPairs/index.ts` and `src/content/setPowers.ts`. Behavior reuses the same targeted-power pipeline as jokers (`src/engine/powers/handlers.ts`, `armedPowerIdForShelf` in `src/lib/powerTargetUi.ts`).

Each deck pair authors its own `powerId`, `initialCharges`, and `initialDuration` on every suit block (deck pairs may diverge later). At present all shipped pairs use the same values per suit: **1 charge**, **permanent** duration (`initialDuration` **null**), with hearts wild, spades transparent, diamonds skip ±1, and clubs card swap.

**Tableau inspect:** When a court card is inspectable and its set key is in **`alignedSetKeys`**, the foundation name plate **Set power** line shows the power **name** from the catalog (e.g. Wild card, Veiled glimpse).

Achievement-gated unique set powers per suit remain **Stage 6+** (see **Achievements**).
...
- End Game — opens a confirmation dialog showing **final score** and **game configuration** (columns, deals, suits (tableau), deck pair, joker count, seed with the same **copy icon beside the seed** as on the game bar), plus **OK** / **Cancel**. **OK** or **Enter** / **Return** (main or keypad) confirms ending the same way, except when keyboard focus is on **Cancel** or the seed **copy** control (those keep their normal behaviour). Confirming replaces the in-progress game with a **cleared board shell** (same config, no cards, no history), leaves the **shelf, foundation row, and stock pile visible (empty)**, and shows an empty tableau for the same column count — **without** opening the New Game dialog (the user opens New Game when they want). That cleared shell is **persisted in `localStorage`** so a **browser refresh** returns to the same post–End Game view (and does not auto-restore an older server save). The user is **not** prompted to save to the server before ending; they can use **Save Game** from the Actions menu when they wish. **Cancel**, **Escape**, or clicking the **backdrop** outside the dialog panel closes the dialog and leaves the game unchanged.
- Logout
...
- The **foundation** sits in the **centre** of the row. To its **left** is a region reserved for the **shelf**; to its **right** is a region reserved for the **stock**. Those two flank columns are **always the same width** (`flankColumnWidthPx` in `src/lib/shelfPanelLayout.ts`, measured with `ResizeObserver` on the top row) and each **uses all horizontal space** available beside the foundation (half of row width minus foundation and gaps), so the foundation stays centred. Each flank is at least wide enough for **`shelfMinWidthPx`** (260px panel) plus **`shelfOuterEdgeGapPx`**. **`shelfFoundationGapPx`** is the grid gap between each flank and the foundation. The **shelf** panel is **horizontally centred** in the left flank and grows with card-strip content up to the space available; **jokers / powers** stay **left-justified** inside the panel. The **stock** pile is **horizontally centred** in the right flank. When content exceeds the panel width, the shelf **scrolls horizontally** inside the panel (mouse wheel over the shelf strip scrolls horizontally when a horizontal scrollbar is shown; trackpad horizontal swipe uses native **deltaX**).
...
- **Name plate (tableau inspect):** Directly **below** the foundation row, **exactly as wide** as the foundation row (**`foundationRowWidthPx()`** = eight card widths plus inter-slot gaps; see **`foundationSlotCount`** in `dimensions.ts`). Fixed **max** height **`foundationNamePlateHeightPx`**, gap **`foundationNamePlateGapPx`**, same styling family as the shelf name plate; long copy **wraps** and the plate **scrolls vertically** when content overflows (scrollbar styling matches the shelf strip). The plate is **empty** when the pointer is not over a tableau card, **foundation** top card, or column badge holder. While a **foundation** top card, **tableau** card, or **column badge holder** is the active inspect target, that target shows a **yellow** ring on its visible edges — **`NAME_PLATE_INSPECT_HIGHLIGHT_CLASS`** (darker) for pip cards and for **face-down** courts **without** transparent, **`NAME_PLATE_INSPECT_FACE_CARD_HIGHLIGHT_CLASS`** (lighter) for **face-up** courts (J/Q/K) and **face-down** courts with **transparent**. When an eligible **court** on the tableau is the inspect target (**face up**, or **face down** with **transparent**), every other **set mate** on the tableau (same **deck** and **suit** as the aligned-set rule) that is also **face up** or **face down** with **transparent** gets the **lighter** court ring; **face-down** set mates **without** transparent do **not** receive the court ring, and a **face-down** opaque court being hovered does **not** trigger set-mate rings on other cards. Tableau inspect also applies when pinned for drag anchor or Shift Card details. While the pointer hovers a **tableau card** with visible identity (**face up**, or **face down** with **transparent** in that column), a **heading** line shows rank and suit (e.g. `King of Spades: Karl Marx - Modern Deck (Blue)` for courts, `6 of Hearts - Classical Deck (Red)` for pip cards). **Tableau** face-down cards without transparent show a **deck-only** heading (`Deck name (Color)`, e.g. `Classical Deck (Red)`) but still hide rank, suit, person, and set; **Card effects** are always listed on card inspect; **Column effects** on card inspect list only entries in `columnEffects` whose id has **`columnAffectsTableauCards`** (not **Extra Column** link timers — those appear only on **column badge holder** inspect). **Foundation** top cards use the full heading when face-up (foundation cards are normally face-up after play). Typography matches the **shelf name plate** (heading **`text-xs`/`font-semibold`**, detail lines **`text-[10px]`**). Below the heading, **two columns** (**1/3** left, **2/3** right): left — **Card effects**, **Column effects**; right — **Set**, **Set power** (right column **blank** unless the card is a court J/Q/K). **Set** uses `{Color} Deck {Suit} - {Deck short name} {Suit theme}` (deck short name drops the trailing word **Deck** from `DeckInPair.name`; `&` in theme names is shown as **and**), e.g. `Blue Deck Clubs - Modern Metaphysics and Ontology`. Each **deck** in a pair has a **`name`** and **`color`** (`red` | `blue`) in `DeckInPair`; **deck 1** is always **red**, **deck 2** always **blue**, and **`color`** selects the shared back art (`back-deck1.png` / `back-deck2.png`) and fallback gradient. Hovering a **column badge holder** shows centered heading **`Column NN`** (1-based, zero-padded, e.g. `Column 01`) and a centered **Column Effects:** line only (no card-effects row). While **dragging** a tableau run, the plate shows the **anchor** card until **drop** or cancel. While **Shift** Card details is open for a tableau card, the plate **pins** that card until the dialog closes (same idea as the shelf name plate for jokers).
...
- Inner layout: **shelfPanelTopInsetPx** above the card row (slightly less than half **shelfVerticalPad** for optical balance), then **cardHeight**, then **shelfHoverScaleBleedPx** and the horizontal scrollbar lane (**shelfHorizontalScrollbarPx**) flush to the panel bottom. The scroll strip uses matching padding and negative horizontal/vertical margin so **hover scale** is not clipped at the inner pad edges (still clipped at the rounded panel border). The grid cell adds **shelfFlankPaddingTopPx** so shelf card tops align with foundation/stock at **shelfVerticalPad**. **shelfPanelHeightPx** = **cardHeight + shelfHorizontalScrollbarPx + shelfPanelTopInsetPx + shelfHoverScaleBleedPx**. **shelfHorizontalPad** insets the strip from the side borders.
- Jokers and set-power instances on the shelf use horizontal step **`max(0, cardWidth − shelfOverlap)`** (first card always flush with the inner left of the shelf). When **`shelfOverlap` &lt; cardWidth**, that step gives about **`shelfOverlap` pixels of overlap** with the card to the left; when **`shelfOverlap` ≥ cardWidth**, step is 0 (cards stacked on one x) and **later cards paint above earlier ones**. If the strip exceeds the panel width, it **scrolls horizontally**.
- **Pointer hover:** The item under the pointer is highlighted only when that item is the **topmost** shelf card at that point (overlapping regions use normal hit testing so a card behind another does not hover). The highlighted item is drawn in front and scaled by **`SHELF_CARD_HOVER_SCALE`** (default matches **110%**) from the **centre** of the card. The scaled card may extend into **`shelfVerticalPad`** / **`shelfHorizontalPad`** but is **clipped at the shelf panel border** (rounded outer edge), not at the inner padding edge. The shelf card strip scrolls horizontally inside a fixed-height **`overflow: hidden`** wrapper so a horizontal scrollbar never grows the strip vertically; **`shelfHoverScaleBleedPx`** padding on the scroll area (with compensating negative margin) keeps hover scale from clipping on the inner edges.
- Contains all jokers that have been dealt and all set power instances that have been created.
- **Engine model:** `game.shelf` is a single `ShelfEntry[]` discriminated union — joker entries `{ kind: "joker", … }` then set-power entries `{ kind: "set", setKey, deckNum, suit, powerId, chargesRemaining }` (partition: all jokers first, then set powers). **`alignedSetKeys`** on game state tracks set keys already awarded a shelf instance. When both blocks are present, **`shelfJokerSetGapPx`** (default **12px** in `dimensions.ts`) is the horizontal distance from the last joker’s right edge to the first set card’s left edge; layout helpers live in `src/lib/setPowerUi.ts` (`shelfEntryLayoutLeftPx`, `shelfStripInnerWidthPx`).
- **Set power card (UI):** `SetPowerShelfCard` in `src/components/game/SetPowerShelfCard.tsx` — same **72×100** shell as jokers. Three court **thumb** portraits from the deck-pair catalog (`courtThumbsForSet` in `src/lib/deckCardArt.ts`), each cropped to the **top half** of the thumb and laid out inside the frame using **`setShelfPortraitInsetPx`**, **`setShelfPortraitBoxHeightPx`**, thumb widths, and row/gap constants in `dimensions.ts`: **King** centred on the top row, **Queen** and **Jack** on the bottom row with a small horizontal gap; the two-row cluster is centred as a unit inside the inset area, then nudged vertically by **`setShelfPortraitClusterOffsetYPx`** (default **10**, positive = up) for optical balance. A shared set frame overlay (`sharedSetFramePath` in `src/constants/gameArtPaths.ts`): **`set-red-frame.svg`** for hearts/diamonds, **`set-black-frame.svg`** for spades/clubs (`public/gameArt/shared/frames/`). Charge badge and depleted wash match jokers (`ShelfChargeBadge`, `shelfDepletedCardWash`).
- **Set power interaction:** Double-click arms or commits/cancels targeted powers the same way as jokers (amber targeting ring, hover scale, invalid cursor while targeting). **Shift+click** Card details applies to **jokers only**, not set shelf cards.
...
- **Name plate:** Directly **below** the shelf panel (fixed height **`shelfNamePlateHeightPx`**, gap **`shelfNamePlateGapPx`**) an empty name plate is always visible. While the pointer hovers a shelf **joker**, it shows that joker’s **name** and **power name** from the deck-pair catalog (same labels as Card details). While the pointer hovers a shelf **set power**, it shows the **set line** (`{Color} Deck {Suit} - {short deck name} {suit theme}`) and the **power name** from the set’s catalog row. When the catalog row’s **`initialDuration`** is set, the power line appends the timed effect length in brackets (e.g. `Extra Column (10 moves)`, `Veiled glimpse (5 moves)`); permanent effects omit the suffix. When a **targeted** power is armed (double-click on a joker or set), the plate **keeps** that item’s labels until the power is committed on a target or cancelled (Escape / Shift / double-click the same shelf item again).

**Game Bar**

- Above the Foundation.
- **Layout:** **Seed** on the **left** (after the initial deal animation completes for a new game or restart, the bar shows an en-dash until then). A **copy icon** beside the seed copies **that same string** to the clipboard when the seed is visible (what you see is what you paste). The **center** shows **Decks:** (deck-pair display name, e.g. Western Philosophy), **Suits:** (`config.numberOfSuits`: **1**, **2**, or **4**), **Powers:** (count of committed power uses — `power_trigger` history entries for **joker and set** powers), **Moves:** (history length), **Undos:** (`undoCount` on game state), and **Score:** (one decimal place, per scoring rules). **Deck** and **Stock** buttons are on the **right**.
...
- **Power Target mode:** The whole bar (including **Deck** / **Stock**) uses the **invalid power-target cursor** (child buttons inherit it; no pointer hand). **Pointer down** or **click** anywhere on the bar (including **Deck** / **Stock**) cancels targeting (no charge) and does **not** open or toggle popups. **Pointer enter** on **Deck** / **Stock** still opens the matching popup when the armed power’s `targetKinds` allow it (see **Powers**).
...
- Each time the popup opens, **columns**, **deals**, **suits (tableau)**, **deck pair**, and **joker count** default to the **last played game’s** values (persisted in the browser when a game is saved or started; **not** cleared when the user ends a game). The **seed** field is always **empty** on open so **Start Game** uses a **new random shuffle** with those layout options unless the user types or pastes a seed. If no prior configuration exists (first visit), defaults are **8** columns, **6** deals, **4** suits, **0** jokers, default deck pair, and an **empty** seed.
- **Seed format:** `CC-DDD-S-XXX-SSSSSSSSSSSSSS` as in **Setup** above (hyphens required; legacy seeds without **S** imply **4** suits). A valid **`ws1:`** replay string is still accepted for backwards compatibility (optional **`numberOfSuits`** in the payload; default **4**).
- When the seed field is **empty**, **Start Game** builds a new formatted seed using the current **columns**, **deals**, **numberOfSuits**, and **deck pair** plus a random 14-digit shuffle key, and the chosen **joker count**.
- When the seed field contains **any** non-whitespace text, **columns**, **deals**, **suits**, and **deck pair** are **disabled** and show the values from the parsed seed (**formatted** or **`ws1:`**). The **joker count** control is **always enabled**; its value is used when starting the game (including for formatted seeds, which do not encode joker count).
...
**End Game:** Choosing **End Game** opens the confirmation dialog directly (no server-save prompt). **OK** or **Enter** / **Return** confirms (except when focus is on **Cancel** or the seed **copy** control). When the user confirms, the in-progress game is replaced by a cleared board shell and that shell is written to **local storage** (same key as in-progress saves); achievements are evaluated when that flow exists.
...
When a deal is performed each card will be separately animated moving face down from the stock to its tableau column. **Escape** or **clicking anywhere** (same exclusions as for the initial deal) during this animation skips any remaining deal flights and commits the final layout (same as when the last flight completes).
