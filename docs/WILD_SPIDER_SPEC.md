...
- **Layout:** **Seed** on the **left** (after the initial deal animation completes for a new game or restart, the bar shows an en-dash until then). A **copy icon** beside the seed copies **that same string** to the clipboard when the seed is visible (what you see is what you paste). The **center** shows **Decks:** (deck-pair display name, e.g. Western Philosophy), **Powers:** (count of committed joker power uses — `power_trigger` history entries), **Moves:** (history length), **Undos:** (`undoCount` on game state), and **Score:** (one decimal place, per scoring rules). **Deck** and **Stock** buttons are on the **right**.
