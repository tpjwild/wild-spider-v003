...
Until per-suit content diverges, all shipped deck pairs use the same catalog row per suit:

| Catalog constant | Persisted `powerId` | Class | Effect | Default charges | Default duration |
|------------------|-------------------|-------|--------|-----------------|------------------|
| `POWER_SELECTED_CARD_TRANSPARENT` | `jokerSelectedCardTransparent` | Targeted | transparent | 10 | 5 moves |

**Tableau inspect:** When a court card is inspectable and its set key is in **`alignedSetKeys`**, the foundation name plate **Set power** line shows the power **name** from the catalog (e.g. Veiled glimpse).
